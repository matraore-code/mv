            ...
            <div class="mt-10 pt-8 border-t border-blueGray-200 text-center">
              <div class="flex flex-wrap justify-center">
                <div class="w-full lg:w-9/12 px-2">
                  <p class="mb-2 text-lg leading-relaxed text-blueGray-700 text-justify">{{ biography }}</p>
                </div>
              </div>
            </div>
    ...
    async mounted() {
      try {
        ...
        const response = await fetch(
          `${"http://10.11.13.5:5000" || "http://localhost:5000"}/api/users/${this.$route.params.profile}`,
          {
            method: "GET",
            headers: {
        ...
        if (content.message) {
          ...
          this.fetched = true;
          this.image = `${"http://10.11.13.5:5000" || "http://localhost:5000"}/${user.image}`;
    ...
    methods: {
      async SaveContact() {
        try {
          ...
          const response = await fetch(
            `${"http://10.11.13.5:5000" || "http://localhost:5000"}/api/contacts/${this.$route.params.profile}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json"
          ...
          if (content.message) {
            ...
            window.location.href = `${"http://10.11.13.5:5000" || "http://localhost:5000"}/${content.link}`;
          }
        } catch (err) {
          console.log(err);
          this.errors = err;
